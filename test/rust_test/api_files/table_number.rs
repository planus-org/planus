...
check_type!(+['a] ExampleRef<'a> => &self.value_null() : planus::Result<Option<u32>>);
check_type!(+['a] ExampleRef<'a> => &self.value_default_zero() : planus::Result<u32>);
check_type!(+['a] ExampleRef<'a> => &self.value_default_one() : planus::Result<u32>);
